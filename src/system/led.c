...
static uint32_t led_compute(enum led_ch ch, enum sys_led_pattern p, uint32_t *state, uint32_t *step_ms)
{
	...
	case SYS_LED_PATTERN_CONNECT_HEARTBEAT: { // 蓝·链路心跳（与绿错峰 2.5s）
		...
		// 多 tracker：日常=充沛度（峰 25%+3.3%×(N-1)）；调试=连跳 N 次（120ms/跳）
		uint32_t peak = 2500 + 330 * (led_tracker_count - 1);
		if (daily) {
			v = breath_shape(phase, 10000, 2500, 2500, peak);
			st = 20;
		} else {
...
// receiver 无 retained 内存：显示偏好持久化走 NVS（system.h 的 LED_MODE_NVS/LED_BRIGHT_NVS）
static void led_thread(void)
{
	uint8_t saved_mode = 0;
	uint8_t saved_bright = 0;
	sys_read(LED_MODE_NVS, &saved_mode, sizeof(saved_mode));
	sys_read(LED_BRIGHT_NVS, &saved_bright, sizeof(saved_bright));
	if (saved_mode == (uint8_t)LED_MODE_DEBUG) {
		led_mode = LED_MODE_DEBUG;
	}
	if (saved_bright >= 5 && saved_bright <= 100) {
		led_brightness_pptt = saved_bright * 100;
	}
...
void set_led_mode(enum led_display_mode mode)
{
	...
	sys_write(LED_MODE_NVS, NULL, &v, sizeof(v));
}
...
void set_led_brightness(uint8_t percent)
{
	...
	sys_write(LED_BRIGHT_NVS, NULL, &percent, sizeof(percent));
}
